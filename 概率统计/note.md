...
$$
若把 $x$ 或 $y$ 的期望看作二维随机变量函数来求解的话，注意积分是双重积分
...
$$
相关系数绝对值越接近 $1$ 时，$X$ 与 $Y$ 的线性相关程度就越强，反之，越接近 $0$ 时，$X$ 与 $Y$ 线性相关程度就越弱，等于零意味着不相关
...
&(4) ~~ \text{正态分布}:~~\text{相互独立}~\Leftrightarrow~ \text{不相关}
\end{align}
$$
...
| $k$ 阶样本**中心矩** | $B_k = \cfrac 1 n \sum\limits_{i = 1}^n(X_i - \overline X)^2$ |
...
<img src="illus/image-20221122下午53031895-5077059-5077085.png" alt="image-20221122下午53031895" style="zoom:25%;" />
$$
...
<img src="illus/image-20221122下午51706043-5077059-5077085.png" alt="image-20221122下午51706043" style="zoom:25%;" />
$$
...
\\~\\
&(4) ~~ T \sim t(n) ~~\Longrightarrow~~ T^2 \sim F(1,n)
\end{align}
$$
...
<img src="illus/image-20221122下午53002758-5077059-5077085.png" alt="image-20221122下午53002758" style="zoom:25%;" />
$$
...
为了降低阶次，方便加速，用对数形式来计算（具有相同的极大值点）
$$
...
设 $\theta$ 为总体的未知参数，$\theta_1$ 和 $\theta_2$ 均为估计量，若对于给定的 $\alpha~(0<\alpha<1)$，满足
$$
...
因为样本的随机性，不免发生误判而犯两类错误

- 第一类错误：「弃真」错误

    $H_0$ 为真，而检验结果降其否定，其犯错的概率为
